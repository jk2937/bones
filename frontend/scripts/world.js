...
Bones.World = {
    ...
    tick() {
        ...
        this.players[netplay_welcome_message].read_keyboard_controls()
        ...
        if(Bones.Input.Keyboard.ControlStates["loadstate"].pressed_this_frame) {
            ...
            client_player.fast_forward(100)
        }
...
Bones.Renderer.context.font = "18px Monospace";
    ...
    PhysicsPlayer: class {
        ...
        load_rewind_buffer(rewind_time=250) {
            ...
            this.control_state_changed = buf.control_state_changed
        ...
        fast_forward(_time=0) {
            //freeze all non player bodies
            for (let i = 0; i < Bones.World.npcs.length; i++) {
                Matter.Body.setStatic(Bones.World.npcs[i].physics_prop.body, true)
            }
            for (let i = 0; i < Object.keys(Bones.World.players).length; i++) {
                let key = Object.keys(Bones.World.players)[i]
                if (key = netplay_welcome_message) {
                    continue
                }
            ...
            for (let i = 0; i < _time; i++) {
                    Matter.Engine.update(Bones.World.Physics.matterjs_engine, 1)
            }
            for (let i = 0; i < Bones.World.npcs.length; i++) {
                Matter.Body.setStatic(Bones.World.npcs[i].physics_prop.body, false)
            }
            for (let i = 0; i < Object.keys(Bones.World.players).length; i++) {
                let key = Object.keys(Bones.World.players)[i]
                if (key = netplay_welcome_message) {
                ...
                Matter.Body.setStatic(Bones.World.players[key].physics_prop.body, false)
            }
